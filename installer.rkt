#lang racket
(require pollen/core net/git-checkout racket/runtime-path "database.rkt" "content.rkt")
(provide installer)
...
(define (installer _ root
  ...
  (define note-root (if (relative-path? dest) (build-path root dest) dest))
  (define xexpr (build-path root "xexpr"))
  (define build (build-path root "build"))

  (define source (build-path note-root "src"))
